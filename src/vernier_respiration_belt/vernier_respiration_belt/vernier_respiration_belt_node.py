...
class ros2_vernier_respiration_belt(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('Device_Name', os.getenv('VERNIER_SERIAL')) # Should be changed inro your device name
        ...
        self.gdx_status = True
        self.recon_att = False
        ...
        self.pub_respiration_belt_force_data = self.create_publisher(Vitals, "biosensors/vernier_respiration_belt/force", 10) #raw Force (N)
    ...
    def vernier_timer_callback(self):
        ...
        if self.Parm_Sensor_Enable:
            ...
            if  np.isnan(self.arr_resp_belt_data[0]) == False:
                vitals = Vitals()
                vitals.stamp = self.get_clock().now().to_msg()
                vitals.data = (self.arr_resp_belt_data[0])
                self.pub_respiration_belt_force_data.publish(vitals)
